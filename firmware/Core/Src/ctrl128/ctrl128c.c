...
volatile static void updatesd(void)
{
	...
	GPIOB->BSRR=LED2_Pin;
	//set file position
	if(*READSD&0x01)*READSD^=1,f_lseek(&fl[*DRIVE],( ((uint32_t)(*TRACK<<1))+(uint32_t)*HEAD )*FLOPPY_DATATRACKSIZE+((uint32_t)(*SECTR-1)<<9));
	//first write
...
static void printstr(uint8_t _nomsel)
{
	if(_nomsel==nomsel)
	{
		fast_string_inv(0,16*_nomsel, " :",(void *)font_8x14);
		fast_putc_inv(0,16*_nomsel,'0'+_nomsel,(void *)font_8x14);
	}
	else
	{
		fast_string(0,16*_nomsel, " :",(void *)font_8x14);
...
static FRESULT getfilename(char *ffname,uint8_t drvsel,uint8_t direct)
{
	...
	if(direct)
	{
		...
		while(res==FR_OK && tmpp<mfile[drvsel].nomfile)
		{
			...
			if (!(fno.fattrib & AM_DIR) && !fast_cmp((void *)fno.fname,(void *)cfgname) )tmpp++,fast_memcpy((void *)ffname,(void *)fno.fname,255);
		}
	}
	else
	{
